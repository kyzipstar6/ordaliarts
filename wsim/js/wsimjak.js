let inc1 =0.01; let inc2=0.005; let inc3= inc1*5; let inc4 =inc2*5;  let accumulator =0; let aci=0;
 let wdirval = 270; let wdir= "West";
...
if (hum>100) hum =89;
    ...
    function tempM(){  
        ...
        if (hour > 10 && hour<18){ if(month ==6 || month==7 ){ temp=30;} if(month ==5 || month==8){ temp=29.5;}if(month ==4 || month==9){ temp=29;}  
        if(month ==3 || month==10){ temp=28;}if(month ==2 || month==11){ temp=28.4;}if(month ==1 || month==12){ temp=28;}}  
        if (hour < 10 || hour>18){  if(month ==6 || month==7 ){temp=23;}if(month ==5 || month==8){ temp=23.5;} if(month ==4 || month==9){ temp=24;}  
        if(month ==3 || month==10){ temp=24;}if(month ==2 || month==11){ temp=24.4;}if(month ==1 || month==12){ temp=24.5;}}  
    ...
    let UPPER_BOUND = 11;let LOWER_BOUND = 1;  
    function wind(){  
        boundSetter();  
        setInterval(() => {  
			...
			if(wspeed1 < UPPER_BOUND && wspeed1 > LOWER_BOUND  && wspeed2 < UPPER_BOUND && wspeed2> LOWER_BOUND && wspeed1/filter <= 0.985  
					) {quickwind = wspeed1;}  accumulator += 1;
	     }, 100);   
    }  
    function boundSetter(){  
        ...
        wdirval +=( 40 - (80*Math.random()));
          if (wdirval >360) wdirval=359;
    if (wdrival<0) wdirval=0; if(wdirval >347 || wdirval<12) wdir = "North";
       if(wdirval >323 || wdirval<=347) wdir = "NNW";  
          if(wdirval >308 && wdirval<=323) wdir = "NW";  
          if(wdirval >281 || wdirval<=308) wdir = "WNW";  
          if(wdirval >258 || wdirval<=281) wdir = "West";
          if(wdirval >235 || wdirval<=258) wdir = "WSW";
          if(wdirval >210 || wdirval<=235) wdir = "SW";
          if(wdirval >192 || wdirval<=210) wdir = "SSW";
          if(wdirval >168 && wdirval<=195) wdir = "S";
          if(wdirval >143 || wdirval<=168) wdir = "SSE";
          if(wdirval >119 || wdirval<=143) wdir = "SE";
          if(wdirval >100 || wdirval<=119) wdir = "ESE";
          if(wdirval >78 && 
             
             wdirval<=100) wdir = "E";
          if(wdirval >55 || wdirval<=78) wdir = "ENE";
        }, 4200);   
